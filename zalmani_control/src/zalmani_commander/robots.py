...
class Robot:
    ...
    def trajectory_publisher(self, goal: FollowJointTrajectoryGoal):
    ...
    def trajectory_cmd(self, trajectory: JointTrajectory):
    ...
    def _cb_active(self):
        ...
        rospy.loginfo(f"{self.controller_name}: received new goal")

    def _cb_feedback(self, msg: FollowJointTrajectoryFeedback):
    ...
    def _cb_done(self, state, result):
        ...
        rospy.loginfo(
            ...
            f"State: {self.result.status}, result: {self.result.result}"
        )

    def _cb_state(self, msg: JointTrajectoryControllerState):
    ...
    def wait_for_result(self, timeout: float = 0.0):
    ...
    def reset(self, time_from_start: float = 0.1):
        """Sets the robot to its initial pose.

        Cancel all goals and publish the initial joint positions on the
        <controller_name>/command topic.

        Args:
            time_from_start: The relative time from the header.stamp
                to the trajectory point.
        """
        self.cancel_all_goals()
        pose = [self.joints[i]['init_position'] for i in self.joints]
        traj = JointTrajectory()
        traj.joint_names = list(self.joints.keys())
        point = JointTrajectoryPoint()
        point.positions = pose
        point.time_from_start = rospy.Duration(time_from_start)
        traj.points.append(point)
        self.trajectory_cmd(trajectory=traj)
...
class UniversalRobots(Robot):
    ...
    def __init__(self, init_pos: list = None, *, effort_controller: bool = False):
        ...
        if not init_pos:
            ...
            rospy.loginfo(f"{topic_name}: Setting all initial joint positions to 0.0")
        if len(init_pos) != 6:
            init_pos = [0.0 for _ in range(6)]
            rospy.loginfo(
